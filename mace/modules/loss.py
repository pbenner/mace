...
def compute_angle(s, t, n1 = None, n2 = None, epsilon=1e-8):
    ...
    return torch.arccos(dp / (n1*n2 + epsilon))
...
def weighted_mean_absolute_virials(ref: Batch, pred: TensorDict) -> torch.Tensor:
    # energy: [n_graphs, ]
    configs_weight = ref.weight.view(-1, 1, 1)  # [n_graphs, ]
    configs_virials_weight = ref.virials_weight.view(-1, 1, 1)  # [n_graphs, ]
    num_atoms = (ref.ptr[1:] - ref.ptr[:-1]).view(-1, 1, 1)  # [n_graphs,]
    return torch.mean(
        configs_weight
        * configs_virials_weight
        * torch.abs((ref["virials"] - pred["virials"]) / num_atoms)
    )  # []
...
def weighted_mean_absolute_error_dipole(ref: Batch, pred: TensorDict) -> torch.Tensor:
    # dipole: [n_graphs, ]
    num_atoms = (ref.ptr[1:] - ref.ptr[:-1]).unsqueeze(-1)  # [n_graphs,1]
    return torch.mean(torch.abs((ref["dipole"] - pred["dipole"]) / num_atoms))  # []
    # return torch.mean(torch.square((torch.reshape(ref['dipole'], pred["dipole"].shape) - pred['dipole']) / num_atoms))  # []
...
class WeightedEnergyForcesLossL1(torch.nn.Module):
    def __init__(self, energy_weight=1.0, forces_weight=1.0) -> None:
        super().__init__()
        self.register_buffer(
            "energy_weight",
...
class WeightedEnergyForcesStressLossL1(torch.nn.Module):
    def __init__(self, energy_weight=1.0, forces_weight=1.0, stress_weight=1.0) -> None:
        super().__init__()
        self.register_buffer(
            "energy_weight",
    ...
    def forward(self, ref: Batch, pred: TensorDict) -> torch.Tensor:
        return (
            ...
            + self.forces_weight * mean_absolute_error_forces(ref, pred)
            + self.stress_weight * weighted_mean_absolute_stress(ref, pred)
        )
